...
class MemoryPoolNotThreadSafety {
  ...
  private:
    ...
    std::mutex mutex_;  // Mutex to guard thread safety
};
...
public:
    ...
    T* Allocate(Args&&... args) {
        ...
        if (block) {
            ...
            if (head_.compare_exchange_strong(block, next, std::memory_order_release, std::memory_order_acquire)) {
    ...
    do {
        // Here we cast obj to a void* and then set the next pointer to the current head
        *reinterpret_cast<void**>(obj) = head;
    } while (!head_.compare_exchange_strong(head, obj, std::memory_order_release, std::memory_order_acquire));
}
    ...
    // Reset the pool by marking all allocated memory as free (without actually purging memory)
    void Reset() {
        pool_.release_memory();
    }

    // Clear the pool by marking all objects as free and returning memory to the pool
    void Clear() {
        pool_.purge_memory();
    }
