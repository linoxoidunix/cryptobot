...
#include "aot/proto/classes/proto_trades.h"
...
class KafkaClient {
  public:
    ...
    class MessageBuilder : public cppkafka::MessageBuilder {
      public:
        explicit MessageBuilder(const Topic& topic)
            : cppkafka::MessageBuilder(static_cast<std::string>(topic)) {}
    };
            ...
            try {
                if constexpr (std::is_same<T, aot::models::OrderBook>::value) {
                    ...
                    producer_->produce(builder_pnl_);
                                                  ...
                                                  aot::models::Trades>::value) {
                    builder_trade_dictionary_.payload(payload);
                    producer_->produce(builder_trade_dictionary_);
                } else {
    ...
    void SendMessageAsJson(const T& object) {
        ...
        try {
            ...
            loge("Failed to serialize object to JSON: {}",
                 std::string(ex.what()));
            return;
        }
            ...
            try {
                ...
                if constexpr (std::is_same<T, aot::models::OrderBook>::value) {
                    ...
                    producer_->produce(builder_trade_);
                                         ...
                                         T, aot::TradeDictionary>::value) {
                    builder_trade_dictionary_.payload(json_string);
                    producer_->produce(builder_trade_dictionary_);
                } else {
                ...
                loge("Error producing JSON message: {}",
                     std::string(ex.what()));
            }
        });
    }
    ...
    void WaitUntilFinished() {
        ...
        boost::asio::post(io_context_, [this]() { producer_->flush(); });
    }
  ...
  private:
    ...
    Topic topic_trade_dictionary_{"trade_dictionary"};
    ...
    MessageBuilder builder_trade_dictionary_{topic_trade_dictionary_};
};

template <typename Executor>
class RedPandaComponent : public bus::Component, public KafkaClient {
  ...
  public:
    ...
    void AsyncHandleEvent(const aot::TradeDictionary& event) override {
        boost::asio::co_spawn(
            executor_,
            [this, &event]() -> boost::asio::awaitable<void> {
  ...
  private:
    boost::asio::awaitable<void> HandleEventAsync(
        ...
        logi("bid:{} ask:{}", wrapped_event->bbo.bid_price,
             wrapped_event->bbo.ask_price);
        auto exchange_id  = wrapped_event->exchange_id;
        auto trading_pair = wrapped_event->trading_pair;

        auto* info =
            exchange_trading_pairs_.GetPairInfo(exchange_id, trading_pair);
        if (!info) {
    ...
    boost::asio::awaitable<void> HandleEventAsync(
        boost::intrusive_ptr<aot::ArbitrageReport> wrapped_event) {
        if (!wrapped_event) {
            logw("Received nullptr wrapped_event in HandleEventAsync");
            co_return;
        }
    ...
    boost::asio::awaitable<void> HandleEventAsync(
        const aot::TradeDictionary& event) {
        aot::models::Trades trades(event, exchange_trading_pairs_);
        SendMessage(trades);
        co_return;
    }
    ...
    aot::models::OrderBook PrepareOrderBook(
        ...
        return aot::models::OrderBook(
            ...
            info.https_json_request, bid_price, ask_price, spread, bid_qty,
            ask_qty);
    }

    template <bool need_bid>
    std::pair<bool, double> GetFormattedPrice(
        boost::intrusive_ptr<Trading::NewBBO> event) {
        if (!event) {
    ...
    template <bool need_bid>
    std::pair<bool, double> GetFormattedQty(
        boost::intrusive_ptr<Trading::NewBBO> event) {
        if (!event) {
