...
class WssSession {
  ...
  public:
    ...
    net::awaitable<bool> AsyncRequest(std::string&& req,
        ...
        co_await stream_.async_close(websocket::close_code::normal, net::use_awaitable);
...
class WssSession2 {
  ...
  private:
    ...
    std::string end_point_;
    std::string request_json_;
    std::list<OnWssResponse*> cbs_;
    boost::asio::io_context& ioc_;
    Timeout timeout_;
    boost::asio::steady_timer timer_;  // Timer to track session expiration
  public:
    virtual ~WssSession2() = default;
    explicit WssSession2(boost::asio::io_context& ioc, 
                        ssl::context& ctx,
                        _Timeout timeout,
                        const std::string_view host,
                        const std::string_view port,
    ...
    net::awaitable<bool> AsyncRequest(std::string&& req,
        ...
        is_used_ = true;
        auto [ec, bytes_written] = co_await stream_.async_write(net::buffer(request_json_), boost::asio::as_tuple(boost::asio::use_awaitable));
        if(ec){
          close_session();
          co_return false;
        }
        ...
        while(need_read_){
          ...
          for (auto& cb_ptr : cbs_) {
            if (cb_ptr) {
                try {
                    // Pass the correct executor from the stream or io_context
                    boost::asio::post(
                        stream_.get_executor(), // Ensure correct executor is used
                        [cb_ptr, this] { 
                            auto& cb = *cb_ptr;
                            cb(buffer_);
                          }
