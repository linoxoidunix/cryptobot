  ...
  private:
        ...
        if (UNLIKELY(!best_orders_by_price)) {
            ...
            if (add_after) {  // add new_orders_at_price after target.
                ...
                if ((new_orders_at_price->side_ == Common::Side::BUY &&
                     new_orders_at_price->price_ >
                         best_orders_by_price->price_) ||
                    (new_orders_at_price->side_ == Common::Side::SELL &&
                     new_orders_at_price->price_ <
                         best_orders_by_price->price_)) {
                    target->next_entry_ =
                        (target->next_entry_ == best_orders_by_price
                             ? new_orders_at_price
                             : target->next_entry_);
                    (new_orders_at_price->side_ == Common::Side::BUY
                         ? bids_by_price_
    ...
    auto removeOrdersAtPrice(Common::Side side, Common::Price price) noexcept {
        ...
        if (UNLIKELY(orders_at_price->next_entry_ ==
            ...
            orders_at_price->prev_entry_ = orders_at_price->next_entry_ =
                nullptr;
        }
    ...
    auto addOrder(MarketOrder *order) noexcept -> void {
        ...
        if (!orders_at_price) {
            ...
            auto first_order =
                (orders_at_price ? orders_at_price->first_mkt_order_ : nullptr);
