...
auto MarketOrderBook::onMarketUpdate(const Exchange::MEMarketUpdate *market_update) noexcept -> void {
    ...
    const auto bid_updated =
        (bids_at_price_map_.size() && market_update->side == Common::Side::SELL &&
         market_update->price >= bids_at_price_map_.begin()->price_);
    const auto ask_updated =
        (asks_at_price_map_.size() && market_update->side == Common::Side::BUY &&
         market_update->price <= asks_at_price_map_.begin()->price_);

    if (market_update->qty != 0) {
        MarketOrder order (Common::OrderId_INVALID, market_update->side, market_update->price, market_update->qty);
        addOrder(&order);
    } else {
