...
int main() {
    ...
    ::V2::ConnectionPool<WSSesionType3, const std::string_view&>
        session_pool_bybit{io_context, WSSesionType3::Timeout{30},
                           3,          "stream.bybit.com",
                           "443",      "/v5/public/spot"};
    ::V2::ConnectionPool<WSSesionType3, const std::string_view&>
        session_pool_binance{io_context, WSSesionType3::Timeout{30},
                             1,          "stream.binance.com",
                             "9443",     "/ws"};
    binance::HttpsConnectionPoolFactory2 factory_binance;
    ::V2::ConnectionPool<HTTPSesionType3>* session_pool =
        factory_binance.Create(io_context, HTTPSesionType3::Timeout{30}, 5,
                               Network::kMainnet, common::MarketType::kSpot,
    ...
    common::TradingPairInfo pair_info_ethbtc_binance{
        .price_precission     = 5,
        .qty_precission       = 4,
        .https_json_request   = "ETHBTC",
        .https_query_request  = "ETHBTC",
        .ws_query_request     = "ethbtc",
        .https_query_response = "ETHBTC"};
    ...
    // Map the trading pair (Exchange ID: 2, Trading Pair ID: 1) to its
    // configuration
    ...
    exchange_trading_pairs.AddOrUpdatePair(common::ExchangeId::kBybit,
                                           btc_usdt_trading_pair,
                                           pair_info_btcusdt_bybit);
    exchange_trading_pairs.AddOrUpdatePair(common::ExchangeId::kBybit,
                                           eth_usdt_trading_pair,
                                           pair_info_ethusdt_bybit);
    exchange_trading_pairs.AddOrUpdatePair(common::ExchangeId::kBybit,
                                           eth_btc_trading_pair,
                                           pair_info_ethbtc_bybit);
    exchange_trading_pairs.AddOrUpdatePair(common::ExchangeId::kBinance,
                                           btc_usdt_trading_pair,
                                           pair_info_btcusdt_binance);
    exchange_trading_pairs.AddOrUpdatePair(common::ExchangeId::kBinance,
                                           eth_usdt_trading_pair,
                                           pair_info_ethusdt_binance);
    exchange_trading_pairs.AddOrUpdatePair(common::ExchangeId::kBinance,
                                           eth_btc_trading_pair,
                                           pair_info_ethbtc_binance);
    exchange_trading_pairs.AddOrUpdatePair(common::ExchangeId::kBinance,
                                           ltc_btc_trading_pair,
                                           pair_info_ltcbtc_binance);
    ...
    binance::BookSnapshotComponent book_snapshot_component_binance(
        ...
        pair_reverse_binance, session_pool, exchange_connection_manager);
    ...
    book_snapshot_component_binance.RegisterCallback(
        btc_usdt_trading_pair, &book_snapshot_calback_handler);
    book_snapshot_component_binance.RegisterCallback(
        eth_usdt_trading_pair, &book_snapshot_calback_handler);
    book_snapshot_component_binance.RegisterCallback(
        eth_btc_trading_pair, &book_snapshot_calback_handler);
    book_snapshot_component_binance.RegisterCallback(
        ltc_btc_trading_pair, &book_snapshot_calback_handler);
    ...
    binance::BookEventGetterComponentCallbackHandler
        book_event_componnent_callback_handler_binance(
            book_event_getter_binance, bid_ask_generator_spot_binance, bus,
    ...
    // Register the callback for the trading pair (Exchange ID: 2, Trading Pair
    // ID: 1)
    book_event_getter_binance.RegisterCallback(btc_usdt_trading_pair,
                                               &wss_cb_binance);
    book_event_getter_binance.RegisterCallback(eth_usdt_trading_pair,
                                               &wss_cb_binance);
    book_event_getter_binance.RegisterCallback(eth_btc_trading_pair,
                                               &wss_cb_binance);
    book_event_getter_binance.RegisterCallback(ltc_btc_trading_pair,
                                               &wss_cb_binance);
    ...
    binance::BidAskGeneratorCallbackBatchHandler
        bid_ask_generator_callback_handler_binance(
            bus, bid_ask_generator_spot_binance);
    // --------------------------Order Book
    // Component-------------------------------- Initialize the order book
    ...
    bus.Subscribe(&bid_ask_generator_spot_binance,
                  &book_snapshot_component_binance);
    bus.Subscribe(&book_snapshot_component_binance,
    ...
    bus.Subscribe(&bid_ask_generator_spot_binance, &book_event_getter_binance);
    bus.Subscribe(&book_event_getter_binance, &bid_ask_generator_spot_binance);
    ...
    // Binance.
    bus.Subscribe(&bid_ask_generator_spot_binance, &order_book_component);

    // Subscribe the order book component to updates from the Red Panda
    ...
    auto intr_bus_request_btcusdt_sub_binance =
        boost::intrusive_ptr<BusEventRequestBBOPrice>(
            &request_bbo_btc_sub_binance);

    bid_ask_generator_spot_binance.AsyncHandleEvent(
        intr_bus_request_btcusdt_sub_binance);
    ...
    auto intr_bus_request_ethusdt_sub_binance =
        boost::intrusive_ptr<BusEventRequestBBOPrice>(
            &request_eth_usdt_sub_binance);

    bid_ask_generator_spot_binance.AsyncHandleEvent(
        intr_bus_request_ethusdt_sub_binance);

    BusEventRequestBBOPrice request_eth_btc_sub_binance;
    request_eth_btc_sub_binance.exchange_id    = common::ExchangeId::kBinance;
    request_eth_btc_sub_binance.trading_pair   = eth_btc_trading_pair;
    request_eth_btc_sub_binance.snapshot_depth = 1000;
    request_eth_btc_sub_binance.subscribe      = true;
    request_eth_btc_sub_binance.id             = 1212L;

    auto intr_bus_request_ethbtc_sub_binance =
        boost::intrusive_ptr<BusEventRequestBBOPrice>(
            &request_eth_btc_sub_binance);

    bid_ask_generator_spot_binance.AsyncHandleEvent(
        intr_bus_request_ethbtc_sub_binance);
    ...
    auto intr_bus_request_ltcbtc_sub_binance =
        boost::intrusive_ptr<BusEventRequestBBOPrice>(
            &request_ltc_btc_sub_binance);

    bid_ask_generator_spot_binance.AsyncHandleEvent(
        intr_bus_request_ltcbtc_sub_binance);
